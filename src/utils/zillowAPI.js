...
export const zillowAPI = async (searchLocation) => {
  const zswid = process.env.REACT_APP_ZILLOW_ZWSID;
  const proxy = "https://cors-anywhere.herokuapp.com/";
  const convert = require("xml-js");
  const results = await fetch(
    `${proxy}http://www.zillow.com/webservice/GetDeepSearchResults.htm?zws-id=${zswid}&address=${searchLocation.streetAddress}&citystatezip=${searchLocation.city}+${searchLocation.state}+${searchLocation.zipCode}`
  );
  const converted = await results.text();
  const cleanJSON = JSON.parse(
    convert.xml2json(converted, { compact: true, spaces: 2 })
  );
  console.log(cleanJSON);
  const dirtyData = 
    cleanJSON["SearchResults:searchresults"].response.results.result;
  const cleanData = {
    zpid: dirtyData.zpid,
